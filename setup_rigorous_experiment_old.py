...
Usage:
    ...
    python setup_rigorous_experiment.py --run_experiment
"""
...
python "$SCRIPTS_DIR/generate_args.py" \\
    "$CONFIGS_DIR/args_config.json" \\
    "$OUTPUTS_DIR/args_responses.json"

# Generate GenARM responses  
echo "Generating GenARM responses..."
python "$SCRIPTS_DIR/generate_genarm.py" \\
    "$CONFIGS_DIR/genarm_config.json" \\
    "$OUTPUTS_DIR/genarm_responses.json"

# Generate Dual-Head responses
echo "Generating Dual-Head responses..."
python "$SCRIPTS_DIR/generate_dual_head.py" \\
    "$CONFIGS_DIR/dual_head_config.json" \\
    "$OUTPUTS_DIR/dual_head_responses.json"
...
def compute_pairwise_comparison(responses_a, responses_b, method_a, method_b):
    ...
    win_rate = win_count / total_comparisons
    return {{
        "method_a": method_a,
        "method_b": method_b,
        "win_rate_a": win_rate,
...
'''
    ...
    if eval_results:
        report += f'''
**Evaluation Results:**
- Methods Evaluated: {{", ".join(eval_results.get("methods_evaluated", []))}}
- Total Comparisons: {{eval_results.get("total_comparisons", 0)}}
- Experiment Status: {{"✅ Completed" if eval_results.get("experiment_completed") else "❌ Incomplete"}}
'''
